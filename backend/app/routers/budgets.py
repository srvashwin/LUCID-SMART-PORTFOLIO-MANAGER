...
def _compute_category_spent(db: Session, user_id: int, month: int, year: int, category: str) -> float:
...
def _build_budget_out(budget: Budget, db: Session, user_id: int) -> BudgetOut:
    cats = db.query(BudgetCategory).filter(BudgetCategory.budget_id == budget.id).all()
    categories_out = []
    total_spent = 0.0
    total_assigned = 0.0
    for bc in cats:
        spent = _compute_category_spent(db, user_id, budget.month, budget.year, bc.category)
...
def list_budgets(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [_build_budget_out(b, db, user.id) for b in budgets]
...
def create_budget(data: BudgetCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if income_value == 0:
        latest = db.query(Income).filter(
            Income.user_id == user.id
        ).order_by(Income.date.desc()).first()
        if latest:
            income_value = latest.amount

    budget = Budget(user_id=user.id, month=data.month, year=data.year, total_income=income_value)
    db.add(budget)
    db.flush()
    ...
    db.commit()
    db.refresh(budget)
    return _build_budget_out(budget, db, user.id)
...
def get_current_budget(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if not budget:
        ...
        budget = Budget(user_id=user.id, month=today.month, year=today.year, total_income=income_value)
        db.add(budget)
        db.flush()
